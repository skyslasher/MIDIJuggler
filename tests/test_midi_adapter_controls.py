...
def test_midi_adapter_publishes_control_events_for_program_change_without_library() -> None:
    async def scenario() -> tuple[list[ControlEvent], list[MidiMessageEvent]]:
        bus = EventBus()
        controls: list[ControlEvent] = []
        messages: list[MidiMessageEvent] = []
        ...
        await adapter._handle_input_message(0xCA, (1,))
        return controls, messages

    controls, messages = asyncio.run(scenario())

    assert len(messages) == 1
    assert messages[0].status == 0xCA
    assert messages[0].data == (1,)
    assert len(controls) == 1
    assert controls[0].source == "xtouch_mini"
    assert controls[0].control == "program_10_1"
